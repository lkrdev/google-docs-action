...
export class GoogleDocsAction extends Hub.OAuthActionV2 {
    ...
    async form(request: Hub.ActionRequest) {
        ...
        if (request.params.state_json) {
            try {
                ...
                if (e.code && e.errors && e.errors[0] && e.errors[0].message) {
                    error = {
                        ...error, http_code: e.code, message: `${errorType.description} [GOOGLE_DOCS] ${e.errors[0].message}`,
                    }
                }
    ...
    private async loginForm(request: Hub.ActionRequest) {
        ...
        form.fields.push({
            ...
            label: "Log in",
            description: "In order to send to Google Docs, you will need to log in" +
