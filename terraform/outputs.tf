output "service_url" {
  ...
  description = "The URL of the deployed Google Docs Action Hub service."
}
...
output "suggested_docker_image_tag" {
  value       = "${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.repo.repository_id}/google-docs-action:latest"
...
1. If you used the default placeholder image, build and push your real Docker image using:
   gcloud builds submit --tag ${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.repo.repository_id}/google-docs-action:latest ..

2. Re-run `terraform apply` with the following variables:
   - image               = "${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.repo.repository_id}/google-docs-action:latest"
